...
function (_ref) {
  ...
  _proto.onDayMouseEnter = function onDayMouseEnter(day) {
    ...
    var modifiers = deleteModifier({}, hoverDate, 'hovered', this.props, this.state);
  ...
  _proto.getFirstFocusableDay = function getFirstFocusableDay(newMonth) {
    ...
    if (isBlocked(focusedDate, this.props)) {
      ...
      var viableDays = days.filter(function (day) {
        return !isBlocked(day, _this2.props) && isAfterDay(day, focusedDate);
      });
  ...
  DayPickerSingleDateController.getModifiers = function getModifiers(visibleDays, modifiers) {
    ...
    Object.keys(visibleDays).forEach(function (month) {
      ...
      visibleDays[month].forEach(function (day) {
        newModifiers[month][toISODateString(day)] = DayPickerSingleDateController.getModifiersForDay(day, modifiers);
      });
    });
    return newModifiers;
  };
  ...
  DayPickerSingleDateController.getStateForNewMonth = function getStateForNewMonth(nextProps, modifiers) {
    ...
    var withoutTransitionMonths = orientation === VERTICAL_SCROLLABLE;
    var visibleDays = DayPickerSingleDateController.getModifiers(getVisibleDays(currentMonth, numberOfMonths, enableOutsideDays, withoutTransitionMonths), modifiers);
    return {
      currentMonth: currentMonth,
      visibleDays: visibleDays
    };
